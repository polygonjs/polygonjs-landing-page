...
<script lang="ts">
import {defineComponent, onBeforeUnmount, onMounted, Ref, ref, computed} from 'vue';
...
const sceneReadyEventName: string = PolyEventName.SCENE_READY.toLowerCase().replace('poly', '');
const viewerMountedEventName: string = PolyEventName.VIEWER_MOUNTED.toLowerCase().replace('poly', '');
const viewerReadyEventName: string = PolyEventName.VIEWER_READY.toLowerCase().replace('poly', '');
...
export default defineComponent({
	...
	emits: [sceneReadyEventName, viewerMountedEventName, viewerReadyEventName, sceneAvailableEventName, 'progress'],
	props: {
		sceneName: {
			type: String,
			required: true,
	...
	setup(props, context) {
		...
		async function loadScene() {
			...
			const configureSceneData = props.configureSceneData as ConfigureSceneData | undefined;
			const domElement = sceneContainer.value;
			domElement.addEventListener(PolyEventName.SCENE_READY, (/*event*/) => {
				context.emit(sceneReadyEventName, scene);
			});
			domElement.addEventListener(PolyEventName.VIEWER_MOUNTED, (event) => {
		...
		function _removePosterAndProgressBar() {
			if (posterContainerRef.value) {
				posterContainerRef.value.parentElement?.removeChild(posterContainerRef.value);
			}
			if (progressBarRef.value) {
				progressBarRef.value.parentElement?.removeChild(progressBarRef.value);
			}
		}
