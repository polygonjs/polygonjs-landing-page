	...
	setup(props, context) {
		...
		async function loadScene() {
			...
			if (loadedData) {
				...
				viewer = loadedData.viewer;
				if (viewer) {
					_createVisibilityObserver(viewer);
				}
			}
		...
		// visibility observer
		function _createVisibilityObserver(_viewer: BaseViewerType) {
			_viewer.updateAutoRenderOnIntersectionChange({
				threshold: 0,
			...
			// 	viewer?.setAutoRender(entry.isIntersecting);
			// }, options);
