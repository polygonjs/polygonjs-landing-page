...
import { requiredImports_player } from "./requiredImports";

const loadSceneFromSceneData_player = async function (options) {
  const {
    domElement,
    sceneData,
    onProgress,
  ...
  if (runRegister) {
    ...
    for (const node of requiredImports_player.nodes) {
      Poly.registerNode(node, undefined, { printWarnings });
    }
    for (const operation of requiredImports_player.operations) {
...
export { Poly, loadSceneFromSceneData_player };
