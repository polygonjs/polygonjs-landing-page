...
import { requiredImports_scene_03 } from "./requiredImports";

const loadSceneFromSceneData_scene_03 = async function (options) {
  const {
    domElement,
    sceneData,
    onProgress,
  ...
  if (runRegister) {
    ...
    for (const node of requiredImports_scene_03.nodes) {
      Poly.registerNode(node, undefined, { printWarnings });
    }
    for (const operation of requiredImports_scene_03.operations) {
      Poly.registerOperation(operation, { printWarnings });
    }
    for (const jsFunction of requiredImports_scene_03.jsFunctions) {
