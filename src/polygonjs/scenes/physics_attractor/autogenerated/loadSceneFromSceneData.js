...
import { requiredImports_physics_attractor } from "./requiredImports";

const loadSceneFromSceneData_physics_attractor = async function (options) {
  const {
    domElement,
    sceneData,
    onProgress,
  ...
  if (runRegister) {
    ...
    for (const node of requiredImports_physics_attractor.nodes) {
      Poly.registerNode(node, undefined, { printWarnings });
    }
    for (const operation of requiredImports_physics_attractor.operations) {
  ...
  const sceneName = "physics_attractor";
...
export { Poly, loadSceneFromSceneData_physics_attractor };
