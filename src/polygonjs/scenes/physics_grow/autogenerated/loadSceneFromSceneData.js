...
import { requiredImports_physics_grow } from "./requiredImports";

const loadSceneFromSceneData_physics_grow = async function (options) {
  const {
    domElement,
    sceneData,
    onProgress,
  ...
  if (runRegister) {
    ...
    for (const node of requiredImports_physics_grow.nodes) {
      Poly.registerNode(node, undefined, { printWarnings });
    }
    for (const operation of requiredImports_physics_grow.operations) {
...
export { Poly, loadSceneFromSceneData_physics_grow };
