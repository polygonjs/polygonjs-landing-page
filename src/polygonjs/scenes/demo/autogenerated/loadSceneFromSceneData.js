...
import { requiredImports_demo } from "./requiredImports";

const loadSceneFromSceneData_demo = async function (options) {
  const {
    domElement,
    sceneData,
    onProgress,
  ...
  if (runRegister) {
    ...
    for (const node of requiredImports_demo.nodes) {
      Poly.registerNode(node, undefined, { printWarnings });
    }
    for (const operation of requiredImports_demo.operations) {
...
export { Poly, loadSceneFromSceneData_demo };
