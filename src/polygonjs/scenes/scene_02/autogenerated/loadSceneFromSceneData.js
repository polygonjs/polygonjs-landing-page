...
import { requiredImports_scene_02 } from "./requiredImports";

const loadSceneFromSceneData_scene_02 = async function (options) {
  const {
    domElement,
    sceneData,
    onProgress,
  ...
  if (runRegister) {
    ...
    for (const node of requiredImports_scene_02.nodes) {
      Poly.registerNode(node, undefined, { printWarnings });
    }
    for (const operation of requiredImports_scene_02.operations) {
      Poly.registerOperation(operation, { printWarnings });
    }
    for (const jsFunction of requiredImports_scene_02.jsFunctions) {
