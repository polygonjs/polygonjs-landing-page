...
import { requiredImports_rhino } from "./requiredImports";

const loadSceneFromSceneData_rhino = async function (options) {
  const {
    domElement,
    sceneData,
    onProgress,
  ...
  if (runRegister) {
    ...
    for (const node of requiredImports_rhino.nodes) {
      Poly.registerNode(node, undefined, { printWarnings });
    }
    for (const operation of requiredImports_rhino.operations) {
...
export { Poly, loadSceneFromSceneData_rhino };
