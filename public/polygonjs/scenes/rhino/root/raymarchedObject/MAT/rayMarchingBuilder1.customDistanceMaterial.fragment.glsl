...
// /raymarchedObject/MAT/rayMarchingBuilder1/fit2
//
//
// FIT
//
...
// /raymarchedObject/MAT/rayMarchingBuilder1/globals2
uniform float time;
...
// /raymarchedObject/MAT/rayMarchingBuilder1/param1
uniform float v_POLY_param_offset;

// /raymarchedObject/MAT/rayMarchingBuilder1/textureSDF1
uniform sampler3D v_POLY_textureSDF_textureSDF1;

// /raymarchedObject/MAT/rayMarchingBuilder1/param2
uniform vec3 v_POLY_param_refractionTint;
...
SDFContext GetDist(vec3 p) {
	...
	// /raymarchedObject/MAT/rayMarchingBuilder1/param1
	float v_POLY_param1_val = v_POLY_param_offset;
	
	// /raymarchedObject/MAT/rayMarchingBuilder1/multAdd2
	float v_POLY_multAdd2_val = (0.59*(v_POLY_globals2_time + 8.9)) + 0.0;
	
	// /raymarchedObject/MAT/rayMarchingBuilder1/multAdd1
	float v_POLY_multAdd1_val = (0.65*(v_POLY_globals1_time + 0.0)) + 0.83;
	...
	float v_POLY_textureSDF1_d = v_POLY_textureSDF1_sdBox < 0.03 ? texture(v_POLY_textureSDF_textureSDF1, v_POLY_textureSDF1_positionNormalised - vec3(0.0, 0.0, 0.0)).r : v_POLY_textureSDF1_sdBox;
	
	// /raymarchedObject/MAT/rayMarchingBuilder1/sin2
	float v_POLY_sin2_val = sin(v_POLY_multAdd2_val);
	
	// /raymarchedObject/MAT/rayMarchingBuilder1/sin1
	float v_POLY_sin1_val = sin(v_POLY_multAdd1_val);
	
	// /raymarchedObject/MAT/rayMarchingBuilder1/multAdd3
	float v_POLY_multAdd3_val = (1.0*(v_POLY_textureSDF1_d + v_POLY_param1_val)) + 0.0;
	
	// /raymarchedObject/MAT/rayMarchingBuilder1/fit2
	float v_POLY_fit2_val = fit(v_POLY_sin2_val, -1.0, 1.0, -0.6, 0.7000000000000001);
	
	// /raymarchedObject/MAT/rayMarchingBuilder1/fit1
	float v_POLY_fit1_val = fit(v_POLY_sin1_val, -1.0, 1.0, 0.1, 1.1);
	
	// /raymarchedObject/MAT/rayMarchingBuilder1/floatToVec3_2
	vec3 v_POLY_floatToVec3_2_vec3 = vec3(v_POLY_fit2_val, 0.0, 0.0);
	
	// /raymarchedObject/MAT/rayMarchingBuilder1/floatToVec3_1
	vec3 v_POLY_floatToVec3_1_vec3 = vec3(0.0, v_POLY_fit1_val, 0.0);
	
	// /raymarchedObject/MAT/rayMarchingBuilder1/SDFBox2
	float v_POLY_SDFBox2_float = sdBox(p - v_POLY_floatToVec3_2_vec3, vec3(0.1, 3.0, 2.0)*1.0);
	
	// /raymarchedObject/MAT/rayMarchingBuilder1/SDFBox1
	float v_POLY_SDFBox1_float = sdBox(p - v_POLY_floatToVec3_1_vec3, vec3(5.0, 0.1, 1.0)*1.0);
	
	// /raymarchedObject/MAT/rayMarchingBuilder1/SDFMin1
	float v_POLY_SDFMin1_min = min(v_POLY_SDFBox2_float, v_POLY_SDFBox1_float);
	
	// /raymarchedObject/MAT/rayMarchingBuilder1/SDFSubtract1
	float v_POLY_SDFSubtract1_subtract = SDFSmoothSubtract(v_POLY_SDFMin1_min, v_POLY_multAdd3_val, 0.03);
	
	// /raymarchedObject/MAT/rayMarchingBuilder1/SDFContext1
	SDFContext v_POLY_SDFContext1_SDFContext = SDFContext(v_POLY_SDFSubtract1_subtract, 0, _RAYMARCHEDOBJECT_MAT_RAYMARCHINGBUILDER1_SDFMATERIAL1, _RAYMARCHEDOBJECT_MAT_RAYMARCHINGBUILDER1_SDFMATERIAL1, 0.);
...
vec3 applyMaterialWithoutRefraction(vec3 p, vec3 n, vec3 rayDir, int mat, inout SDFContext sdfContext){
	...
	// /raymarchedObject/MAT/rayMarchingBuilder1/param2
	vec3 v_POLY_param2_val = v_POLY_param_refractionTint;
